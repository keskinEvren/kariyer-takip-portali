...
namespace KariyerTakip
{
    public class WorkHistoryController : Controller
    {
        ...
        public WorkHistoryController(ApplicationDbContext context, UserManager<User> userManager)
        {
            _context = context;
        ...
        {
            var selam = await _context.WorkHistory
        ...
        public async Task<IActionResult> Create([Bind("Id,Company,Description,UserId")] WorkHistory workHistory)
        {
            if (ModelState.IsValid)
            {
                ...
                workHistory.UserId = Guid.Parse(_userManager.GetUserId(User));
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, [Bind("Id,Company,Description")] WorkHistory workHistory)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    var existingWorkHistory = await _context.WorkHistory.FindAsync(id);
                    ...
                    existingWorkHistory.Company = existingWorkHistory.Company;
                    existingWorkHistory.Description = existingWorkHistory.Description;
